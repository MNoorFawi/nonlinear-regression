...
GAM model in its simplest form, tries to relax the linearity constraint and finds a set of functions s_i() and a constant term a0 such that
f(x[i,]) = a0 + s_1(x[i,1]) + s_2(x[i,2]) + ... s_n(x[i,n])

Let's look at an example;
we first synthesize the data.
```{r data}
library(ggplot2)
set.seed(13)
x <- rnorm(1000)
...
the RMSE score is so improved.
let's now look at the other way.
